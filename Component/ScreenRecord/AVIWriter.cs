...
namespace ScreenRecord
{
    internal class AVIWriter
    {
        ...
        internal Bitmap Create(string fileName, UInt32 frameRate, int width, int height)
        {
        ...
        internal void AddFrame()
        {
            ...
            if (hr != 0) throw new AVIException("AVI流添加帧错误");
            _bmp.UnlockBits(bmpDat);
            _bmp.Dispose();
        ...
        internal void LoadFrame(Bitmap nextframe)
        {
            _bmp = nextframe;
        }
        ...
        /// </summary>
        /// <param name="nextframe">图片</param>
        internal void AddFrame(Bitmap nextframe)
        {
            _bmp = nextframe;
            AddFrame();
        }
        ...
        private void CreateStream()
        {
            ...
            if (hr != 0)
            {
                throw new AVIException("AVI流创建错误");
            }
        }
        ...
        unsafe private void SetOptions()
        {
            ...
            if (hr != 0)
            {
                throw new AVIException("AVI压缩流错误");
            }
            ...
            if (hr != 0)
            {
                throw new AVIException("AVI流设置格式错误", hr);
            }
        }
    }
}